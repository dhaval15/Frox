...
local function create_boxed_widget(widget_to_be_boxed, width, height, bg_color)
    ...
    box_container.bg = beautiful.start_tile_background
...
awful.spawn.easy_async_with_shell("hostname", function(out)
    ...
    out = out:gsub('^%s*(.-)%s*$', '%1')
    host_text.markup = helpers.colorize_text("@"..out, beautiful.start_text)
end)
...
local function decorate_cell(widget, flag, date)
    ...
    local default_fg = beautiful.start_text
